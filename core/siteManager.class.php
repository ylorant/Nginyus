...
class NginyUS_SiteManager extends NginyUS_SystemPages
{
	...
	public function __construct(&$main)
	{
		...
		$this->defaultSite->setName('default');
	}
	...
	public function pageCall($id, $data)
	{
		...
		NginyUS::message('Checking matches in sites for $0...', array($data['host']), E_DEBUG);
		...
		if($exec)
		{
			NginyUS::message('Found host match in $0...', array($name), E_DEBUG);
			$localDir = getcwd();
		...
		if(!$found)
		{
			NginyUS::message('Page $0 not found.', array($data['page']), E_WARNING);
			$errorFound = FALSE;
			if($exec)
			{
				$localDir = getcwd();
